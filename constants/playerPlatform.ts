...
export type ShowcaseRsvpStatus = 'Invited' | 'Attending' | 'Declined' | 'Completed';
...
export type ShowcaseEvent = {
  ...
  confirmedClubs: string[];
  pitchType: string;
  registrationDeadline: string;
  rsvpStatus: ShowcaseRsvpStatus;
  assignedSquad: string;
  matchSchedule: ShowcaseMatch[];
  notes: string;
  createdAt: string;
};
...
export type ShowcaseMetrics = {
  ...
  invitedCount: number;
  totalScoutsAttending: number;
  uniqueClubsRepresented: number;
...
export type ShowcaseReport = {
  ...
  showcases: ShowcaseEvent[];
};

export const defaultShowcaseReport: ShowcaseReport = {
  playerId: 'demo-player',
  metrics: {
    totalShowcases: 3,
    attendingCount: 1,
    invitedCount: 2,
    totalScoutsAttending: 54,
    uniqueClubsRepresented: 16,
    showcaseIndex: 85,
  },
  showcases: [
    {
...
export type TransferStatus = 'Free Agent' | 'Under Contract' | 'Loan Listed' | 'Transfer Listed' | 'Academy Prospect';

export type MandateStatus = 'Open' | 'Reviewing' | 'Closed';
...
export type MarketValuation = {
  ...
  valueRangeMinEur: number;
  valueRangeMaxEur: number;
  marketTier: string;
  demandIndex: number;
  valuationFactors: ValuationFactor[];
  historicalGrowth: ValuationHistoryPoint[];
};
...
export const defaultTransferReport: TransferReport = {
  playerId: 'demo-player',
...
export type InterviewTargetClub = {
  id: string;
  club: string;
  country: string;
...
export type InterviewPrepReport = {
  ...
  targetClubs: InterviewTargetClub[];
  modules: InterviewModule[];
  questionsBank: InterviewQuestion[];
  simulationSessions: SimulationSession[];
};

export const defaultInterviewPrepReport: InterviewPrepReport = {
  playerId: 'demo-player',
